    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ BACK", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(text=f"🤖 Check Bot PM 🤖", url=f"https://t.me/{temp.U_NAME}"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('Status', url='https://t.me/+Ak47fj5R6E03ZmE1'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"</b> ✍️ Note:</b> <s>This message will be Auto-deleted after 10 hours to avoid copyright issues.</s>"
...
async def advantage_spell_chok(msg):
    ...
    if not g_s:
        ...
        await k.delete()
        return
    regex = re.compile(r".*(imdb|google).*", re.IGNORECASE)  # look for imdb / google results
    ...
    if not movielist:
        ...
        [[
         ...
         [
           InlineKeyboardButton("🔍 IMDB 🔎", url="https://www.imdb.com/find?q={search}"),
        ]])
        k = await msg.reply(f"Hello 👋 {mention} I Couldn't 🔍 Find <b>{search}</b> You Asked For 🤷\n\n<s>Click [GOOGLE] [IMDB] On Any Button And Find The Correct Movie/Series Name And Enter It Here ⤵️</s>\n\n</s>If You Do Not Receive The Movie/Series Even After Entering The Correct Name Then Your Requested Movie/Series Does Not Exit In My Database 🗄</s>", reply_markup=button)
